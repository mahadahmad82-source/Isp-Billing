  ...
  return (
    <div className={`min-h-screen p-4 pb-24 ${pageBg}`}>
      <div className="flex items-start justify-between mb-5">
        <div>
          <h1 className={`text-2xl font-black ${text}`}>🗺️ Collection Route Sheet</h1>
          <p className={`text-xs mt-0.5 ${muted}`}>Agent ke liye printable collection list</p>
        </div>
      ...
      <div className={`border rounded-2xl p-4 mb-4 ${card}`}>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={() => setGroupBy('area')} className={btn(groupBy==='area')}>📍 Area</button>
            <button onClick={() => setGroupBy('status')} className={btn(groupBy==='status')}>📋 Status</button>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`text-xs font-bold ${muted}`}>Include:</span>
            <button onClick={() => setShowExpired(!showExpired)} className={btn(showExpired)}>❌ Expired</button>
            <button onClick={() => setShowExpiring(!showExpiring)} className={btn(showExpiring)}>⏳ Expiring</button>
          </div>
          {showExpiring && (
      ...
      {targetUsers.length === 0 ? (
        <div className={`text-center py-16 ${muted}`}>
          ...
          <p className="font-bold">Koi customer nahi</p>
          <p className="text-xs mt-1">Filter adjust karo</p>
        </div>
      ) : (
          ...
          {grouped.map(([group, list]) => (
            <div key={group} className={`border rounded-2xl overflow-hidden ${card}`}>
              <div className={`px-4 py-3 flex items-center justify-between ${isDark ? 'bg-white/8' : 'bg-slate-50'}`}>
                <span className={`font-black text-sm ${text}`}>📍 {group}</span>
