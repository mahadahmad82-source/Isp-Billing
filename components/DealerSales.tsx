  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-6 md:p-7 rounded-3xl shadow-sm">
        ...
        <div className="flex flex-wrap gap-2 mt-6 border-t border-slate-100 dark:border-white/5 pt-4">
          {(['overview', 'purchases', 'sales', 'products'] as View[]).map(item => (
            <button key={item} onClick={() => setView(item)} className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${view === item ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-white/5'}`}>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div className="bg-amber-500/5 border border-amber-500/10 p-5 rounded-[2rem]"><p className="text-[10px] font-bold uppercase tracking-widest text-amber-500/70">Cost of Goods</p><p className="text-2xl md:text-3xl font-black text-amber-500 mt-1">{money(monthlyCost)}</p><p className="text-[9px] uppercase tracking-widest text-amber-500/50 mt-2">Weighted average cost</p></div>
        <div className={`p-5 rounded-[2rem] border ${monthlyProfit >= 0 ? 'bg-emerald-500/5 border-emerald-500/10' : 'bg-rose-500/5 border-rose-500/20'}`}><p className={`text-[10px] font-bold uppercase tracking-widest ${monthlyProfit >= 0 ? 'text-emerald-500/70' : 'text-rose-500/70'}`}>Gross Profit</p><p className={`text-2xl md:text-3xl font-black mt-1 ${monthlyProfit >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>{money(Math.abs(monthlyProfit))}</p><p className={`text-[9px] uppercase tracking-widest mt-2 ${monthlyProfit >= 0 ? 'text-emerald-500/50' : 'text-rose-500/50'}`}>{margin}% margin</p></div>
        <div className="bg-slate-900 text-white dark:bg-white dark:text-slate-900 p-5 rounded-[2rem]"><p className="text-[10px] font-bold uppercase tracking-widest opacity-60">Stock on Hand</p><p className="text-2xl md:text-3xl font-black mt-1">{totalStockUnits.toLocaleString()} units</p><p className="text-[9px] uppercase tracking-widest opacity-50 mt-2">Value {money(stockValue)}</p></div>
      </div>
