          ...
          <div className="flex items-center gap-3 sm:gap-6">
            ...
            <div className="hidden sm:flex bg-slate-100 dark:bg-white/5 p-1 rounded-xl">
              ...
              <button 
                ...
                className={`px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${
                  activePortalTab === 'complaints' ? 'bg-white dark:bg-white/10 text-indigo-600 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                Complaints
              </button>
      ...
      <div className="sm:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-sm">
        <div className="flex items-center justify-around bg-slate-900/90 dark:bg-slate-800/90 backdrop-blur-xl border border-white/10 p-2 rounded-[2rem] shadow-2xl shadow-indigo-500/20">
          ...
          <button 
            onClick={() => setActivePortalTab('complaints')}
            className={`flex flex-col items-center gap-1 flex-1 py-2 transition-all ${
              activePortalTab === 'complaints' ? 'text-white' : 'text-slate-400'
            }`}
          >
            <div className={`p-1.5 rounded-full transition-all ${activePortalTab === 'complaints' ? 'bg-indigo-600 scale-110 shadow-lg shadow-indigo-600/30' : ''}`}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/></svg>
            </div>
            <span className="text-[8px] font-black uppercase tracking-widest">Complaints</span>
          </button>
        </div>
      </div>
