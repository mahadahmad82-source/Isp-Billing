...
const Login: React.FC<LoginProps> = ({ onLogin, onBack }) => {
  ...
  const getHeading = () => {
    ...
    if (view === 'signup-tier' && tierPaymentPending) return { title: `${tierPaymentPending.label} Plan`, sub: 'Pay using any option below, then upload your receipt' };
    if (view === 'signup-tier') return { title: 'Choose Your Plan', sub: 'Free starts instantly. Paid plans activate after verification.' };
  ...
  return (
    <div className="login-page min-h-screen relative flex items-center justify-center p-4 overflow-hidden bg-[#f4f7fc]">
      ...
      <div className={`w-full relative z-[10] ${view === 'signup-tier' ? 'max-w-md' : 'max-w-sm'}`}>
