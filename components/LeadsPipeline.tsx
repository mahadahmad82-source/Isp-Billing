    ...
    <div className="min-h-screen bg-[#0b0f1a] text-white p-4 pb-24">
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-bold text-white/50 uppercase tracking-wider block mb-2">Naam *</label>
            <input value={form.name||''} onChange={e => setForm(p=>({...p,name:e.target.value}))}
              placeholder="Customer naam" className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-3 text-white text-sm focus:outline-none focus:border-indigo-500"/>
          </div>
          <div>
            <label className="text-xs font-bold text-white/50 uppercase tracking-wider block mb-2">Phone *</label>
            <input value={form.phone||''} onChange={e => setForm(p=>({...p,phone:e.target.value}))}
              placeholder="03xxxxxxxxx" className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-3 text-white text-sm focus:outline-none focus:border-indigo-500"/>
          </div>
        </div>

        <div>
          <label className="text-xs font-bold text-white/50 uppercase tracking-wider block mb-2">Address</label>
          <input value={form.address||''} onChange={e => setForm(p=>({...p,address:e.target.value}))}
            placeholder="Ghar ka address" className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500"/>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-bold text-white/50 uppercase tracking-wider block mb-2">Area</label>
            <input value={form.area||''} onChange={e => setForm(p=>({...p,area:e.target.value}))}
              placeholder="Mohalla / Colony" className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-3 text-white text-sm focus:outline-none focus:border-indigo-500"/>
          </div>
          <div>
            <label className="text-xs font-bold text-white/50 uppercase tracking-wider block mb-2">Interested Plan</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-bold text-white/50 uppercase tracking-wider block mb-2">Source</label>
          ...
          <div>
            <label className="text-xs font-bold text-white/50 uppercase tracking-wider block mb-2">Assign To Agent</label>
        ...
        {form.source === 'Referral' && (
          <div>
            <label className="text-xs font-bold text-white/50 uppercase tracking-wider block mb-2">Referred By</label>
            <input value={form.referredBy||''} onChange={e => setForm(p=>({...p,referredBy:e.target.value}))}
              placeholder="Customer naam jo refer kiya" className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-3 text-white text-sm focus:outline-none focus:border-indigo-500"/>
          </div>
        )}

        <div>
          <label className="text-xs font-bold text-white/50 uppercase tracking-wider block mb-2">Follow-up Date</label>
        ...
        <div>
          <label className="text-xs font-bold text-white/50 uppercase tracking-wider block mb-2">Note</label>
      ...
      <div className="min-h-screen bg-[#0b0f1a] text-white p-4 pb-24">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 mb-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            {detail.area && <div className="bg-white/5 rounded-xl p-3"><p className="text-white/40 text-xs">Area</p><p className="font-semibold mt-1">{detail.area}</p></div>}
            {detail.interestedPlan && <div className="bg-white/5 rounded-xl p-3"><p className="text-white/40 text-xs">Plan</p><p className="font-semibold mt-1">{detail.interestedPlan}</p></div>}
            {detail.source && <div className="bg-white/5 rounded-xl p-3"><p className="text-white/40 text-xs">Source</p><p className="font-semibold mt-1">{detail.source}</p></div>}
            {detail.assignedTo && <div className="bg-white/5 rounded-xl p-3"><p className="text-white/40 text-xs">Agent</p><p className="font-semibold mt-1">{detail.assignedTo}</p></div>}
            {detail.referredBy && <div className="col-span-2 bg-white/5 rounded-xl p-3"><p className="text-white/40 text-xs">Referred By</p><p className="font-semibold mt-1">{detail.referredBy}</p></div>}
            {detail.followUpDate && (
          ...
          {detail.note && <div className="mt-3 bg-white/5 rounded-xl p-3 text-sm text-white/60 italic">"{detail.note}"</div>}
        </div>
        ...
        <div className="flex gap-3 mt-1">
          <button onClick={() => { setForm({...detail}); setEditId(detail.id); setView('edit'); }}
            className="flex-1 py-3 bg-white/5 border border-white/10 rounded-2xl font-bold text-sm">✏️ Edit</button>
    ...
    <div className="min-h-screen bg-[#0b0f1a] text-white p-4 pb-24">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-white/40 text-xs mt-0.5">New connection inquiries track karo</p>
        </div>
        ...
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-2xl p-4 mb-4">
          ...
          {todayFollowUps.map(l => (
            <button key={l.id} onClick={() => { setDetail(l); setView('detail'); }}
              ...
              {l.name} — {l.phone} <span className="text-white/40 text-xs ml-1">({STATUS_CONFIG[l.status].label})</span>
            </button>
          ))}
        </div>
      )}
        ...
        {(['new','contacted','survey_done','install_pending','converted','lost'] as LeadStatus[]).map(s => {
          ...
          return (
            <button key={s} onClick={() => setFilterStatus(filterStatus === s ? 'all' : s)}
              ...
              <p className="text-[9px] font-bold text-white/50 uppercase tracking-wider mt-0.5 leading-tight">{cfg.label.replace(' ✅','')}</p>
            </button>
          );
        })}
      </div>
      ...
      {leads.length > 0 && (
        <div className="bg-gradient-to-r from-emerald-600/20 to-teal-600/20 border border-emerald-500/20 rounded-2xl px-5 py-3 mb-4 flex items-center justify-between">
          <span className="text-xs font-bold text-white/50 uppercase tracking-wider">Conversion Rate</span>
      ...
      <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search naam, phone, area..."
        className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 placeholder-white/30 mb-4"/>
              ...
              <button key={lead.id} onClick={() => { setDetail(lead); setView('detail'); }}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/50 text-sm">{lead.phone}</p>
                    {lead.area && <p className="text-white/30 text-xs mt-1">📍 {lead.area}</p>}
      ...
      {confirmConvert && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 bg-slate-900 border border-white/10 rounded-3xl p-8 w-full max-w-sm text-center">
            ...
            <p className="text-white/40 text-sm mb-6">{confirmConvert.name} ko active customer mein add kiya jayega.</p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmConvert(null)} className="flex-1 py-3 bg-white/5 rounded-2xl font-bold text-sm">Cancel</button>
              <button onClick={() => {
