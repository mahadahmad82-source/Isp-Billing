...
import { AntennaIcon, TargetIcon, MapPinIcon, NoEntryIcon, BoltIcon, BarChartIcon, ClipboardIcon, LockIcon } from './icons/UiIcons';
...
const UPGRADE_INFO: Record<string, { icon: React.FC<{ className?: string }>; desc: string; tier: string }> = {
  ...
  leads:      { icon: TargetIcon, desc: 'Manage your new-connection inquiry pipeline', tier: 'Business' },
  area:       { icon: MapPinIcon, desc: 'View customers and revenue by area', tier: 'Business' },
  suspension: { icon: NoEntryIcon, desc: 'Connection suspend/restore with full log', tier: 'Business' },
  outage:     { icon: BoltIcon, desc: 'Track network downtime', tier: 'Business' },
  analytics:  { icon: BarChartIcon, desc: 'Revenue trends and business insights', tier: 'Business' },
  systemlogs: { icon: ClipboardIcon, desc: 'Full system activity log', tier: 'Business' },
};
...
const UpgradeGate: React.FC<Props> = ({ sub, feature, featureName, children }) => {
  ...
  const info = UPGRADE_INFO[feature] || { icon: LockIcon, desc: featureName, tier: 'Business' };
    ...
    <div className="min-h-screen bg-[#0b0f1a] text-white flex items-center justify-center p-6">
      <div className="max-w-sm w-full text-center">
        ...
        <div className="w-24 h-24 rounded-3xl bg-gradient-to-br from-indigo-600/20 to-purple-600/20 border border-indigo-500/20 flex items-center justify-center mx-auto mb-6">
          <InfoIcon className="w-10 h-10 text-indigo-300" />
        </div>
        ...
        <div className="space-y-3 mb-8">
          {[
            ...
            <div key={plan.tier} className={`bg-gradient-to-r ${plan.color} border rounded-2xl p-4 flex items-center justify-between`}>
              ...
              <a href={`https://wa.me/923042773453?text=I want the MYISP ${plan.label} plan — manager: ${sub.managerId}`}
        ...
        <p className="text-white/30 text-xs">WhatsApp us: 0304-2773453</p>
      </div>
    </div>
  );
};
