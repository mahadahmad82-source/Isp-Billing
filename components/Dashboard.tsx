...
import { shareToWhatsApp, sendWhatsAppDirect, sendExpiryTemplate } from '../utils/whatsapp';
...
const Dashboard: React.FC<DashboardProps> = ({ users, receipts, settings, businessExpenses = [], onDeleteReceipt, setActiveTab, onSetUserFilter, onSetExpiredFilter, pendingRemindersCount = 0, onLogout, isAdmin = false, onUpdateUser, managerId = 'mahadnet' }) => {
  const [activeModal, setActiveModal] = useState<ModalType>(null);
  const [sendingMetaId, setSendingMetaId] = useState<string | null>(null);
  ...
  // Meta-approved "Package Expiry" template — works outside the 24h WhatsApp
  // session window, unlike the plain-text WA reminder button.
  const handleSendMetaExpiryTemplate = async (u: UserRecord) => {
    if (!u.phone || sendingMetaId === u.id) return;
    setSendingMetaId(u.id);
    const expiryDisplay = u.expiryDate ? new Date(u.expiryDate).toLocaleDateString('en-PK', { day: '2-digit', month: 'short', year: 'numeric' }) : '-';
    const r = await sendExpiryTemplate(u.phone, u.name, expiryDisplay, u.plan, managerId);
    setSendingMetaId(null);
    if (!r.success) alert(r.error || 'Meta template send failed — check WhatsApp Business API / Meta billing status.');
  };
    ...
    return (
      <div className="p-3.5 rounded-2xl bg-black/20 border border-white/10 hover:bg-black/30 transition-all flex items-center justify-between gap-3">
        ...
        <div className="flex gap-1.5 flex-shrink-0">
          ...
          <button onClick={() => handleSendMetaExpiryTemplate(u)} disabled={sendingMetaId === u.id}
            className="px-2 py-2 bg-violet-500/80 hover:bg-violet-500 disabled:opacity-50 text-white rounded-xl text-[10px] font-black uppercase tracking-wider transition-all active:scale-95"
            title="Send Meta-approved expiry template (works outside 24h window)">
            {sendingMetaId === u.id ? '...' : 'META'}
          </button>
          {onUpdateUser && (
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 transition-colors">
      ...
      {/* ── Today Expiry & Expired Cards (count only — tap/View List opens full list modal) ── */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Today Expiry */}
        <div onClick={() => setActiveModal('TODAY_EXPIRY')}
          className="bg-gradient-to-br from-amber-500 to-orange-600 rounded-[2.5rem] p-7 shadow-2xl relative overflow-hidden cursor-pointer group active:scale-95 duration-200 transition-all">
          ...
          <div className="flex items-center gap-3 mb-6 relative z-10">
            <div className="w-14 h-14 bg-white/20 rounded-3xl flex items-center justify-center text-white transition-transform group-hover:scale-110">
          ...
          <p className="relative z-10 text-6xl font-black text-white tracking-tight leading-none mb-6">
            {todayExpiringUsers.length}
          </p>
          <button onClick={(e) => { e.stopPropagation(); setActiveModal('TODAY_EXPIRY'); }}
            className="relative z-10 w-full py-3 bg-white/20 hover:bg-white/30 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2">
            View List
        ...
        {/* Today Expired */}
        <div onClick={() => setActiveModal('TODAY_EXPIRED')}
          className="bg-gradient-to-br from-rose-600 to-red-800 rounded-[2.5rem] p-7 shadow-2xl relative overflow-hidden cursor-pointer group active:scale-95 duration-200 transition-all">
          <div className="absolute top-0 right-0 w-40 h-40 bg-white/10 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none"></div>
          <div className="flex items-center gap-3 mb-6 relative z-10">
            <div className="w-14 h-14 bg-white/20 rounded-3xl flex items-center justify-center text-white transition-transform group-hover:scale-110">
              <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button onClick={(e) => { e.stopPropagation(); setActiveModal('TODAY_EXPIRED'); }}
            className="relative z-10 w-full py-3 bg-white/20 hover:bg-white/30 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2">
            View List
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 5l7 7-7 7" /></svg>
          </button>
